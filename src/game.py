...
class Game(object):
	...
	def __init__(self):
		...
		self.thrust_key = None
		self.left_turn_key = None
		self.right_turn_key = None
		self.fire_key = None
		self.IFF_key = None
		self.shots_key = None
		self.pnts_key = None
	...
	def draw_world(self):
		...
		if not self.config['Hexagon']['hide_big_hex']:
			self.bighex.draw()
		if self.config['Fortress']['fortress_exists'] and not self.fortress.alive:
			pass
		else:
	...
	def log_world(self):
		...
		if self.ship and self.ship.alive:
			...
			ship_health = "NA"
			distance = "NA"
		if self.mine_list and len(self.mine_list) > 0:
			mine_no = self.mine_list.mine_count
			mine_id = self.mine_list[0].iff
			fl = " ".join(self.mine_list.foe_letters)
		else:
			mine_no = "NA"
			mine_id = "NA"
			fl = "NA"
		if self.fortress and self.config['Fortress']['fortress_exists'] and self.fortress.alive:
			fortress_alive = "y"
		...
		if not self.score or self.score.iff == '':
			iff_score = 'NA'
		else:
			iff_score = self.score.iff

		keys = pygame.key.get_pressed()
		if self.thrust_key and keys[self.thrust_key]:
			thrust_key = "y"
		else:
			thrust_key = "n"
		if self.left_turn_key and keys[self.left_turn_key]:
			left_key = "y"
		else:
			left_key = "n"
		if self.right_turn_key and keys[self.right_turn_key]:
			right_key = "y"
		else:
			right_key = "n"
		if self.fire_key and keys[self.fire_key]:
			fire_key = "y"
		else:
			fire_key = "n"
		if self.IFF_key and keys[self.IFF_key]:
			iff_key = "y"
		else:
			iff_key = "n"
		if self.shots_key and keys[self.shots_key]:
			shots_key = "y"
		else:
			shots_key = "n"
		if self.pnts_key and keys[self.pnts_key]:
			pnts_key = "y"
		else:
			pnts_key = "n"

		data = ["STATE", system_time, game_time, self.current_game, "NA", "NA", "NA", "NA",
				fl, ship_alive, ship_health, ship_x, ship_y,
		...
		if self.score:
			data = data + [self.score.pnts, self.score.cntrl, self.score.vlcty,
						   ...
						   self.score.mines, self.mine2, self.score.bonus]
		else:
			data = data + ["NA", "NA", "NA", "NA", "NA", "NA", "NA", "NA", "NA", "NA", "NA", "NA", "NA", "NA"]

		data = data + [thrust_key, left_key, right_key, fire_key, iff_key, shots_key, pnts_key]
